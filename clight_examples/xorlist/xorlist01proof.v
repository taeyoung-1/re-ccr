...
Section PROOF.
  ...
  Lemma sim_add_tl :
    sim_fnsem wf top2
      ("add_tl", fun_to_tgt "xorlist" (GlobalStb (Sk.canon sk)) (mk_pure add_tl_spec))
      ("add_tl", cfunU (decomp_func (Sk.canon sk) ce f_add_tl)).
  Proof.
    Opaque encode_val.
    Opaque cast_to_ptr.
    econs; ss. red.
  ...
  Lemma sim_delete_tl :
    sim_fnsem wf top2
      ("delete_tl", fun_to_tgt "xorlist" (GlobalStb (Sk.canon sk)) (mk_pure delete_tl_spec))
      ("delete_tl", cfunU (decomp_func (Sk.canon sk) ce f_delete_tl)).
  Proof.
    Opaque encode_val.
    econs; ss. red.
    ...
    iSplitL "INV tl_ofs"; iFrame.
    { iPureIntro. red. rewrite m_tl_size. vm_compute (size_chunk Mptr). vm_compute (Ptrofs.unsigned Ptrofs.zero). nia. }
    ...
    - ss. destruct v; clarify.
      ...
      vm_compute (rev [Vlong tl_prev_item]). ss.
