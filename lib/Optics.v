...
Module Store.
  ...
  (* Definition t := (Any.t * (Any.t -> Any.t))%type. *)
  ...
  (* Definition counit : t -> Any.t :=
    fun x => snd x (fst x). *)

  Definition cojoin {S A} : t S A -> t S (t S A) :=
    fun x => (fst x, fun a' => (a', snd x)).

  (* Definition cojoin : t -> t :=
    fun x => (fst x, fun a' => Any.pair a' (snd x)). *)
...
Module Lens.
  ...
  Lemma set_view (l: t) : forall s, set l (view l s) s = s.
  Proof.
    destruct l as [l [H1 H2]]. unfold view, set; ss.
    i. hspecialize H1 with s. ss.
  Qed.

  Lemma set_set (l : t) : forall v v' s, set l v' (set l v s) = set l v' s.
  Proof.
    destruct l as [l [H1 H2]]. unfold view, set; ss.
    i. hspecialize H2 with s. cong snd in H2. hspecialize H2 with v. ss.
    unfold compose in H2. rewrite H2. ss.
...
(* Module Lens.
  ...
  Lemma compose_assoc (l1 : t) (l2 : t) (l3 : t) :
    (compose (compose l1 l2) l3) = compose l1 (compose l2 l3).
  Proof.
    eapply eq_sig_hprop.
    - i. eapply proof_irrelevance.
